...
class SalesOrder(models.Model):
    STATUS_CHOICES = [
        ...
        ('pending', '待审核'),
        ('confirmed', '已审核'),
        ('partially_delivered', '部分出库'),
        ('done', '已完成'),
        ('cancelled', '已取消'),
    ]

    order_no = models.CharField(max_length=50, unique=True, verbose_name='销售单号')
    customer = models.ForeignKey('commons.Partner', on_delete=models.PROTECT,
                                 limit_choices_to={'type__in': ['customer', 'both']},
                                 verbose_name='客户')
    warehouse = models.ForeignKey('inventory.Warehouse', on_delete=models.PROTECT, verbose_name='出货仓库')
...
class SalesOrderLine(models.Model):
    order = models.ForeignKey(SalesOrder, on_delete=models.CASCADE, related_name='lines', verbose_name='销售订单')
    ...
    delivered_qty = models.DecimalField(max_digits=12, decimal_places=2, default=0, verbose_name='已出库数量')
